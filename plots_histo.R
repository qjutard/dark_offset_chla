...
mirrored_hist(off_auto_sample[which(auto_to_med_sample<cutoff)], off_med_sample[which(auto_to_med_sample<cutoff)], 10, "mg/m³", "Automatic", "Median")
...
mirrored_hist <- function(x1, x2, br, xlab, ylab1, ylab2) {
    par(mfrow=c(2,1))
    ...
    #Make the plot
    par(mar=c(0.1,5,3,3))
    hist(x1 , main="" , xlim=Xrange, xlab="", ylab=ylab1, ylim=Yrange , xaxt="n", las=1 , col="slateblue1", breaks=br)
    par(mar=c(5,5,0.1,3))
    hist(x2 , main="" , xlim=Xrange, xlab=xlab, ylab=ylab2, ylim=rev(Yrange) , las=1 , col="tomato3"  , breaks=br)
